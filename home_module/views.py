...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        most_visit_products = Product.objects.filter(is_active=True,
                                                     is_delete=False).annotate(visit_count=Count('productvisit')).order_by('-visit_count')[:12]
        context['latest_products'] = group_list(latest_products)
        context['most_visit_products'] = group_list(most_visit_products)
        categories = list(ProductCategory.objects.filter(is_active=True, is_delete=False).annotate(products_count=Count('product_categories')).filter(is_active=True, is_delete=False, products_count__gt=0)[:6])
        categories_products = []
        ...
        #annotate be har satr table ye item append mikone alan order_count append mikone
...
def mobile_menu(request):
    return render(request,'shared/mobile_menu.html')
